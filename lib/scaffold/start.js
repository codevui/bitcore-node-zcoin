...
function checkConfigVersion2(fullConfig) {
    ...
    if (!datadirUndefined || addressDefined || dbDefined) {
        ...
        console.warn('\nConfiguration file is not compatible with this version. \n' +
            ...
            'There are changes necessary in both bitcoin.conf and bitcore-node-zcoin.json. \n\n' +
            'To upgrade please see the details below and documentation at: \n' +
            'https://github.com/bitpay/bitcore-node-zcoin/blob/bitcoind/docs/upgrade.md \n');
 ...
 * @param {Object} options.config - The parsed bitcore-node-zcoin.json configuration file
...
function start(options) {
    ...
    fullConfig.path = path.resolve(options.path, './bitcore-node-zcoin.json');
    console.log('fullConfig.path = ' + fullConfig.path + '\n');
...
function loadModule(req, service) {
    try {
        // first try in the built-in bitcore-node-zcoin services directory
